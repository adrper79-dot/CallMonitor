...
export default function CallDetailView({
  ...
  if (!call) {
    return <div className="p-4 bg-gray-50 rounded-md text-gray-500 text-center">Call not found</div>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className="text-xs text-gray-400 font-mono">ID: {call.id.slice(0, 8)}...</div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm border-t border-gray-100 pt-4">
          ...
          {call.ended_at && (
            <div>
              ...
              <div className="text-gray-900">{new Date(call.ended_at).toLocaleString()}</div>
            </div>
          )}
        </div>
      </section>
      ...
      {['completed', 'failed', 'no-answer', 'busy', 'in_progress'].includes(call.status || '') &&
        organizationId && (
          <section aria-label="Outcome Declaration">
            <OutcomeDeclaration
              callId={call.id}
              organizationId={organizationId}
              callCompleted={['completed', 'failed', 'no-answer', 'busy'].includes(
                call.status || ''
              )}
      ...
      {transcript &&
        (transcript.sentiment_analysis || transcript.entities || transcript.chapters) && (
      ...
      {organizationId && (
        <section
          aria-labelledby="modulations-used"
          className="p-4 bg-white rounded-md border border-gray-200"
        >
