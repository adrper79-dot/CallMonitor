...
- [x] **Standards Audit** (`scripts/standards-audit.ts`): 5-check ARCH_DOCS compliance scanner — org isolation, RBAC, SQL injection, snake_case, audit log. 0 violations / 105 files. ✅
- [x] **Architecture Validation** (`scripts/validate-architecture.ts`): Route/lib/cron count verification against APPLICATION_FUNCTIONS.md. 5/5 checks pass. ✅
- [x] **Post-Deploy Health** (`scripts/post-deploy-health.ts`): 8 production smoke checks — health, deep health, auth, CORS, rate limit, cron health, schema health, webhook DLQ. ✅
- [x] **Workflow Validation Tests** (`tests/production/workflow-validation.test.ts`): 8 critical business workflow tests (auth→RBAC, collections, campaigns, multi-channel, CRM, webhooks, billing, compliance). ✅
