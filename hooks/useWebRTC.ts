...
const debug =
  process.env.NODE_ENV === 'development'
    ? (...args: unknown[]) => console.info('[Telnyx]', ...args)
    : (..._args: unknown[]) => {}
...
export type CallState = 'idle' | 'dialing' | 'ringing' | 'active' | 'ending'
...
export function useWebRTC(organizationId: string | null): UseWebRTCResult {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      debug('Audio element created/configured:', audio.id)
    }
    return () => {
  ...
  useEffect(() => {
    if (callState === 'active' && currentCall) {
      durationIntervalRef.current = setInterval(() => {
        setCurrentCall((prev) =>
          prev
            ? { ...prev, duration: Math.floor((Date.now() - prev.started_at.getTime()) / 1000) }
            : null
        )
      }, 1000)
    } else {
    ...
    async function listAudioDevices() {
      try {
        ...
        audioInputs.forEach((device, i) => {
          debug(`Device ${i}: "${device.label}" (${device.deviceId.substring(0, 8)}...)`)
        })
        debug('Default device will be the first one unless specified')
      } catch (e) {
  ...
  const performConnect = useCallback(async () => {
    ...
    try {
      ...
      debug('Fetching WebRTC credentials...')
      ...
      debug('Credential obtained:', tokenRes.credential_id)
      debug('Caller ID:', tokenRes.caller_id)
      ...
      debug('Got credentials, initializing client...')
      ...
      debug('Creating client with token:', tokenRes.token.substring(0, 50) + '...')
      debug('Token length:', tokenRes.token.length, 'chars')
      ...
      socketEvents.forEach((evt) => {
        client.on(evt, () => debug(`Event: ${evt}`))
      })

      client.on('socket.close', (data: any) => {
        debug('WebSocket closed:', data)
      })
      ...
      client.on('socket.message', (message: any) => {
        debug('WebSocket message:', message?.method || message)
      })

      // Try both event name formats
      client.on('telnyx.ready', () => {
        debug('telnyx.ready event')
      ...
      client.on('telnyx.notification', (notification: any) => {
        debug('Notification:', notification?.type, notification)

        // Handle callUpdate notifications
        if (notification?.type === 'callUpdate' && notification?.call) {
          const call = notification.call
          const state = call.state
          debug('Call state from notification:', state, 'direction:', call.direction)
          ...
          switch (state) {
            ...
            case 'answering':
              ...
              setCurrentCall({
                ...
                phone_number:
                  call.options?.destinationNumber || call.options?.remoteCallerNumber || 'Unknown',
                started_at: new Date(),
                duration: 0,
              })
      ...
      function attachRemoteAudio(call: any) {
        try {
          ...
          if (!remoteStream && call.peer?.pc) {
            ...
            debug('PC receivers:', receivers.length)
            ...
            if (audioReceiver?.track) {
              ...
              debug('Created stream from receiver track')
            }
          }

          debug('Attempting to attach remote audio, stream:', remoteStream)

          if (remoteStream) {
            // Log stream details
            const audioTracks = remoteStream.getAudioTracks()
            debug('Audio tracks in stream:', audioTracks.length)
            audioTracks.forEach((track: MediaStreamTrack, i: number) => {
              debug(
                `Track ${i}: enabled=${track.enabled}, muted=${track.muted}, readyState=${track.readyState}`
              )
              ...
              if (playPromise) {
                playPromise
                  ...
                  .catch((e: Error) => {
                    ...
                    debug('Added click listener to resume audio')
                  })
              }
            }
          } else {
      ...
      function debugLocalAudio(call: any) {
        try {
          debug('=== LOCAL AUDIO DEBUG ===')

          // Check call object for local stream
          const localStream = call.localStream || call.peer?.localStream || call._localStream
          debug('Local stream from call:', localStream)

          if (localStream) {
            const audioTracks = localStream.getAudioTracks()
            debug('Local audio tracks:', audioTracks.length)
            audioTracks.forEach((track: MediaStreamTrack, i: number) => {
              debug(
                `Local track ${i}: enabled=${track.enabled}, muted=${track.muted}, readyState=${track.readyState}, label=${track.label}`
              )
            })
          }
          ...
          if (call.peer?.pc) {
            ...
            debug('PC Senders:', senders.length)
            senders.forEach((sender: RTCRtpSender, i: number) => {
              if (sender.track) {
                debug(
                  `Sender ${i}: kind=${sender.track.kind}, enabled=${sender.track.enabled}, muted=${sender.track.muted}, readyState=${sender.track.readyState}`
                )
              } else {
                debug(`Sender ${i}: no track`)
              }
            })
          }

          // Check if call has mute state
          debug('Call mute state:', {
      ...
      client.on('login', async () => {
        debug('Login event - refreshing token')
        try {
          // Fetch new token
          const tokenRes = await apiGet<{
            success: boolean
          ...
          if (tokenRes.success && tokenRes.token) {
            ...
            debug('Reconnected with refreshed token')
          } else {
      ...
      client.on('authFailed', async () => {
        debug('Auth failed - attempting full reconnect')
        try {
      ...
      client.on('callUpdate', (call: any) => {
        debug('Call update:', call.state, call)
      ...
      client.on('media', (media: any) => {
        debug('Media event received:', media)
        if (media?.stream && remoteAudioRef.current) {
          debug('Attaching media stream from event')
          remoteAudioRef.current.srcObject = media.stream
          remoteAudioRef.current
            .play()
            .catch((e: Error) => console.error('[Telnyx] Audio play error', e))
        }
      })

      client.on('telnyx.media', (media: any) => {
        debug('telnyx.media event received:', media)
        if (media?.stream && remoteAudioRef.current) {
          debug('Attaching telnyx.media stream')
          remoteAudioRef.current.srcObject = media.stream
          remoteAudioRef.current
            .play()
            .catch((e: Error) => console.error('[Telnyx] Audio play error', e))
      ...
      client.on('track', (track: any) => {
        debug('Track event received:', track)
      })

      // 5. Connect - wrap in try/catch for better error handling
      debug('About to call client.connect()...')
      try {
        const connectResult = await client.connect()
        debug('connect() returned:', connectResult)
      } catch (connectErr: any) {
        console.error('[Telnyx] connect() threw:', connectErr)
        throw connectErr
      }
      debug('Connection initiated, waiting for ready event...')
  ...
  const makeCall = useCallback(
    async (phoneNumber: string) => {
      ...
      try {
        ...
        debug('Initiating WebRTC call to:', phoneNumber, 'from:', callerId)
        ...
        try {
          ...
          const audioInputs = devices.filter((d) => d.kind === 'audioinput')
          debug(
            'Available audio devices for call:',
          ...
          if (realMic && realMic.deviceId) {
            debug('Using real microphone:', realMic.label)
            audioConstraint = {
            ...
            debug('No non-virtual mic found, using default with constraints')
          ...
          debug('Could not enumerate devices, using default audio')
        }
        ...
        debug('WebRTC call created:', call?.id || 'no id')
