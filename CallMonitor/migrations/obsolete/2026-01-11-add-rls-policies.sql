...
CREATE POLICY "users_select_own" ON public.users FOR SELECT USING (auth.user_equals_auth(id::text));
...
CREATE POLICY "users_update_own" ON public.users FOR UPDATE USING (auth.user_equals_auth(id::text));
