...
describeOrSkip('Production Database Tests', () => {
  ...
  describe('Session Management', () => {
    test('authjs schema exists', async () => {
      const tables = await query(`
        SELECT table_name 
        FROM information_schema.tables 
        WHERE table_schema = 'authjs'
      `)
    ...
    test('can create and query sessions', async () => {
      // AuthJS uses "sessionToken" (camelCase) and requires UUID for userId
      // First, check if we have a UUID user in authjs.users for testing
      const authUsers = await query(`
        SELECT id FROM "authjs"."users" LIMIT 1
      `)
      
      if (authUsers.length === 0) {
        console.log('⚠️ No AuthJS users exist, skipping session test')
        return
      }
      ...
      // Cleanup
      await query(`
        DELETE FROM "authjs"."sessions" WHERE "sessionToken" = $1
      `, [sessionToken])
