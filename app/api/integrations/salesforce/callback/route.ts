    ...
    try {
        ...
        const upsertRes = await pgClient.query(
            `INSERT INTO integrations (organization_id, provider, provider_account_id, provider_account_name, status, connected_at, connected_by, updated_at)
             ...
             RETURNING *`,
            [stateData.orgId, 'salesforce', tokenResult.accountInfo?.orgId ?? null, tokenResult.accountInfo?.userName ?? null, 'active', new Date().toISOString(), stateData.userId, new Date().toISOString()]
        )
