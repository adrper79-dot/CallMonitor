...
export default function CallModulations({
  ...
  const [unlocked, setUnlocked] = useState<boolean>(() => {
    try {
      return sessionStorage.getItem(`unlock:${callId}`) === '1'
    } catch {
      return false
    }
  })
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (j?.success) {
        try {
          sessionStorage.setItem(`unlock:${callId}`, '1')
        } catch {}
        setUnlocked(true)
      } else {
  ...
  const items = [
    ...
    {
      key: 'synthetic_caller' as ModulationKey,
      label: 'Synthetic Caller',
      hint: 'Inject synthetic caller',
    },
  ]
        ...
        <div className="mb-4 p-3 bg-slate-800 rounded">
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            <div className="text-xs text-slate-400">
              Unlocking uses a guarded API; in production set UNLOCK_USER/UNLOCK_PASS to enable.
            </div>
          </div>
        </div>
      ) : null}
        ...
        <div className="grid grid-cols-1 gap-3">
          {items.map((item) => {
            ...
            return (
              <div
                key={item.key}
                className="flex items-center justify-between px-2 py-2 rounded-md hover:bg-slate-800"
              >
                <div className="flex items-center space-x-2">
                  <Switch
                    id={item.key}
