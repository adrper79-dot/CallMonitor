...
sequenceDiagram
    ...
    COE->>SW: Initiate call (LaML/SWML)
    SW-->>COE: Call started (call_id)
    COE->>DB: Update call status
    COE-->>UI: Success + call_id
    
    Note over SW,User: Call in progress...
...
│   ├── ✅ Voice config (tab)
│   ├── ✅ AI control (tab)
│   ├── ⚠️  AI agent config (NEW - partial UI)
│   ├── ✅ Survey builder (tab)
│   ├── ✅ Secret shopper (tab)
│   ├── ✅ Compliance/retention (tab)
│   ├── ✅ Team management (tab)
│   └── ⚠️  Billing (tab - basic UI only)
│       ├── ✅ Usage display (NEW)
│       ├── ⚠️  Billing actions (stub)
│       └── ❌ Self-service upgrade (missing)
│
├── /review/{callId}
...
```
❌ /analytics
   ├── Dashboard exists as widgets in /dashboard
   ├── No dedicated analytics page
   └── Backend API exists (/api/analytics/*)

❌ /webhooks
   ├── Backend fully implemented
   ├── webhook_subscriptions table exists
   └── No UI for configuration

⚠️  /settings?tab=billing
...
graph TB
    ...
    subgraph "External APIs"
        SIGNALWIRE[SignalWire<br/>Voice & Media]
...
2. AI Agent Config UI Polish (1 day)
   - Complete plan-based feature locking
   - Testing with live SignalWire agents
...
3. Dedicated Analytics Page (3-4 days)
   - Move dashboard widgets to /analytics
   - Add call volume trends
...
| Live Translation | 100% | 70% | 60% | 90% | ⚠️ 80% |
...
| **Stripe Billing** ⭐ | **100%** | **30%** | **70%** | **80%** | ⚠️ **65%** |
| **AI Agent Config** ⭐ | **100%** | **92%** | **85%** | **90%** | ⚠️ **92%** |
| Analytics Dashboard | 80% | 40% | 70% | 60% | ⚠️ 60% |
| Webhook Config UI | 100% | 0% | 80% | 70% | ⚠️ 50% |

**Legend:**
- ✅ 90-100% = Fully complete
- ⚠️ 60-89% = Partial implementation
- ❌ 0-59% = Incomplete
- ⭐ = New implementation (Jan 16, 2026)
...
Auth: NextAuth.js 4.24 with Supabase Adapter
...
### **External Services**
```yaml
Voice: SignalWire (LaML for standard, SWML for AI)
...
Cron Jobs: Vercel Cron (scheduled calls)
