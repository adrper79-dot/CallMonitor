...
- **File:** `workers/src/routes/auth.ts` — CSRF endpoint generates a token, but the login endpoint only checks if _any_ token is present — never compares it to the issued one
...
Your customers are organizations that make business calls (property management, legal, healthcare, government). When they have issues, they shouldn't have to call _you_. The platform should help them.
...
_"The system captures commitments. Let's make sure it can keep its own." — CIO_
