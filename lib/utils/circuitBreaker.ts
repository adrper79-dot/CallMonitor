...
export class CircuitBreaker {
  ...
  private async executeWithTimeout<T>(fn: () => Promise<T>): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(
          new Error(`${this.config.vendorName} request timeout after ${this.config.timeout}ms`)
        )
      }, this.config.timeout)
  ...
  private transitionToClosed(): void {
    ...
    logger.info(`${this.config.vendorName} circuit breaker CLOSED - service recovered`, {
      ...
      downtime:
        previousState === 'OPEN'
          ? Math.floor((Date.now() - this.metrics.stateChangedAt) / 1000)
          : 0,
    })
  }
  ...
  getHealthStatus(): {
    ...
    const errorRate =
      this.metrics.totalCount > 0 ? (this.metrics.failureCount / this.metrics.totalCount) * 100 : 0
