...
CREATE TABLE public.ai_agent_audit_log (
  ...
  changed_by uuid,
  change_type text NOT NULL CHECK (change_type = ANY (ARRAY['created', 'updated', 'deleted', 'enabled', 'disabled'])),
  old_config jsonb,
  new_config jsonb,
  change_reason text,
...
CREATE TABLE public.artifact_provenance (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  artifact_type text NOT NULL CHECK (artifact_type = ANY (ARRAY['recording', 'transcript', 'translation', 'survey', 'score', 'evidence_manifest', 'evidence_bundle'])),
  artifact_id uuid NOT NULL,
  parent_artifact_id uuid,
  parent_artifact_type text,
  produced_by text NOT NULL CHECK (produced_by = ANY (ARRAY['system', 'human', 'model'])),
...
CREATE TABLE public.attention_decisions (
  ...
  decision text NOT NULL CHECK (decision = ANY (ARRAY['escalate', 'suppress', 'include_in_digest', 'needs_review'])),
...
CREATE TABLE public.attention_events (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  event_type text NOT NULL CHECK (event_type = ANY (ARRAY['call_completed', 'alert_triggered', 'webhook_failed', 'carrier_degraded', 'campaign_ended', 'evidence_generated', 'system_error'])),
...
CREATE TABLE public.attention_policies (
  ...
  name text NOT NULL,
  description text,
  policy_type text NOT NULL CHECK (policy_type = ANY (ARRAY['quiet_hours', 'threshold', 'recurring_suppress', 'keyword_escalate', 'custom'])),
...
CREATE TABLE public.audit_logs (
  ...
  after jsonb,
  created_at timestamp with time zone DEFAULT now(),
  actor_type text CHECK (actor_type = ANY (ARRAY['human', 'system', 'vendor', 'automation'])),
  actor_label text,
...
CREATE TABLE public.call_confirmation_checklists (
  ...
  organization_id uuid NOT NULL,
  status text NOT NULL DEFAULT 'pending' CHECK (status = ANY (ARRAY['pending', 'confirmed', 'declined', 'skipped', 'not_applicable'])),
...
CREATE TABLE public.call_confirmations (
  ...
  call_id uuid NOT NULL,
  organization_id uuid NOT NULL,
  confirmation_type text NOT NULL CHECK (confirmation_type = ANY (ARRAY['disclosure_accepted', 'recording_consent', 'terms_agreed', 'price_confirmed', 'scope_confirmed', 'identity_verified', 'authorization_given', 'understanding_confirmed', 'custom'])),
  confirmation_label text,
  prompt_text text NOT NULL,
  confirmer_role text NOT NULL CHECK (confirmer_role = ANY (ARRAY['customer', 'operator', 'third_party', 'both'])),
  confirmed_at timestamp with time zone NOT NULL DEFAULT now(),
  recording_timestamp_seconds numeric,
  captured_by text NOT NULL DEFAULT 'human' CHECK (captured_by = ANY (ARRAY['human', 'system'])),
  captured_by_user_id uuid,
  verification_method text CHECK (verification_method = ANY (ARRAY['verbal', 'keypress', 'biometric', 'document', 'other'])),
  notes text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.caller_id_default_rules (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  scope_type text NOT NULL CHECK (scope_type = ANY (ARRAY['organization', 'user', 'role'])),
  user_id uuid,
  role_scope text,
...
CREATE TABLE public.caller_id_numbers (
  ...
  created_at timestamp with time zone DEFAULT now(),
  created_by uuid,
  status text NOT NULL DEFAULT 'active' CHECK (status = ANY (ARRAY['active', 'suspended', 'retired'])),
...
CREATE TABLE public.caller_id_permissions (
  ...
  permission_type text NOT NULL DEFAULT 'use' CHECK (permission_type = ANY (ARRAY['use', 'manage', 'full'])),
  is_active boolean NOT NULL DEFAULT true,
...
CREATE TABLE public.calls (
  ...
  created_by uuid,
  call_sid text,
  disposition text CHECK (disposition = ANY (ARRAY['sale', 'no_answer', 'voicemail', 'not_interested', 'follow_up', 'wrong_number', 'other'])),
  disposition_set_at timestamp with time zone,
  disposition_set_by uuid,
  consent_method text CHECK (consent_method = ANY (ARRAY['ivr_played', 'verbal_yes', 'dtmf_confirm', 'written', 'assumed', 'none'])),
  ...
  immutability_policy text NOT NULL DEFAULT 'limited' CHECK (immutability_policy = ANY (ARRAY['immutable', 'limited', 'mutable'])),
  custody_status text NOT NULL DEFAULT 'active' CHECK (custody_status = ANY (ARRAY['active', 'archived', 'legal_hold', 'expired'])),
  retention_class text NOT NULL DEFAULT 'default' CHECK (retention_class = ANY (ARRAY['default', 'regulated', 'legal_hold'])),
  legal_hold_flag boolean NOT NULL DEFAULT false,
  evidence_completeness text NOT NULL DEFAULT 'unknown' CHECK (evidence_completeness = ANY (ARRAY['unknown', 'partial', 'complete', 'failed'])),
  disclosure_type text CHECK (disclosure_type = ANY (ARRAY['recording', 'survey', 'translation', 'qa_evaluation', 'multi'])),
...
CREATE TABLE public.campaign_calls (
  ...
  status text NOT NULL DEFAULT 'pending' CHECK (status = ANY (ARRAY['pending', 'calling', 'completed', 'failed', 'canceled'])),
  attempt_number integer NOT NULL DEFAULT 1,
  max_attempts integer NOT NULL DEFAULT 3,
  outcome text CHECK (outcome = ANY (ARRAY['answered', 'no_answer', 'busy', 'failed', 'error'])),
  duration_seconds integer,
  error_message text,
  score_data jsonb,
...
CREATE TABLE public.campaigns (
  ...
  name text NOT NULL,
  description text,
  status text NOT NULL DEFAULT 'draft' CHECK (status = ANY (ARRAY['draft', 'scheduled', 'active', 'paused', 'completed', 'canceled'])),
  call_flow_type text NOT NULL CHECK (call_flow_type = ANY (ARRAY['secret_shopper', 'survey', 'outbound', 'test'])),
  target_list jsonb NOT NULL DEFAULT '[]',
  caller_id_id uuid,
  script_id uuid,
  survey_id uuid,
  custom_prompt text,
  schedule_type text NOT NULL DEFAULT 'immediate' CHECK (schedule_type = ANY (ARRAY['immediate', 'scheduled', 'recurring'])),
...
CREATE TABLE public.carrier_status (
  ...
  status text NOT NULL CHECK (status = ANY (ARRAY['operational', 'degraded', 'outage'])),
...
CREATE TABLE public.compliance_restrictions (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  restriction_code text NOT NULL UNIQUE CHECK (restriction_code = ANY (ARRAY['QA_NO_CONFIRMATIONS', 'QA_NO_OUTCOMES', 'QA_NO_AGREEMENTS', 'SURVEY_NO_AGREEMENTS', 'AI_NO_NEGOTIATION'])),
  restriction_name text NOT NULL,
  description text NOT NULL,
  is_active boolean DEFAULT true,
  violation_action text NOT NULL DEFAULT 'warn' CHECK (violation_action = ANY (ARRAY['block', 'warn', 'log'])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.compliance_violations (
  ...
  violation_type text NOT NULL CHECK (violation_type = ANY (ARRAY['blocked', 'warned', 'detected', 'prevented'])),
  violation_context jsonb,
  resolution_status text DEFAULT 'open' CHECK (resolution_status = ANY (ARRAY['open', 'reviewed', 'dismissed', 'confirmed'])),
...
CREATE TABLE public.confirmation_templates (
  ...
  use_cases ARRAY DEFAULT ARRAY['general'],
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  created_by uuid,
...
CREATE TABLE public.crm_object_links (
  ...
  call_id uuid NOT NULL,
  crm_object_type text NOT NULL CHECK (crm_object_type = ANY (ARRAY['contact', 'company', 'deal', 'lead', 'account', 'opportunity'])),
  crm_object_id text NOT NULL,
  crm_object_name text,
  crm_object_url text,
  synced_at timestamp with time zone,
  sync_direction text NOT NULL CHECK (sync_direction = ANY (ARRAY['inbound', 'outbound'])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.crm_sync_log (
  ...
  organization_id uuid NOT NULL,
  integration_id uuid NOT NULL,
  operation text NOT NULL CHECK (operation = ANY (ARRAY['oauth_connect', 'oauth_disconnect', 'oauth_refresh', 'push_evidence', 'push_note', 'push_engagement', 'pull_contact', 'pull_company', 'pull_deal', 'link_object', 'unlink_object', 'error', 'rate_limited'])),
  status text NOT NULL CHECK (status = ANY (ARRAY['pending', 'success', 'failed', 'rate_limited', 'skipped'])),
  call_id uuid,
  ...
  completed_at timestamp with time zone,
  triggered_by text NOT NULL CHECK (triggered_by = ANY (ARRAY['user', 'system', 'webhook', 'scheduler'])),
...
CREATE TABLE public.digests (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  digest_type text NOT NULL CHECK (digest_type = ANY (ARRAY['overnight', 'weekly', 'on_demand'])),
...
CREATE TABLE public.disclosure_logs (
  ...
  organization_id uuid NOT NULL,
  call_id uuid,
  disclosure_type text NOT NULL CHECK (disclosure_type = ANY (ARRAY['recording', 'survey', 'translation', 'qa_evaluation', 'multi'])),
  disclosure_text text NOT NULL,
  disclosed_at timestamp with time zone NOT NULL DEFAULT now(),
  disclosure_method text NOT NULL DEFAULT 'tts' CHECK (disclosure_method = ANY (ARRAY['tts', 'prerecorded', 'ivr', 'agent'])),
...
CREATE TABLE public.evidence_bundles (
  ...
  immutability_policy text NOT NULL DEFAULT 'immutable' CHECK (immutability_policy = ANY (ARRAY['immutable', 'limited', 'mutable'])),
  tsa jsonb,
  tsa_status text NOT NULL DEFAULT 'not_configured' CHECK (tsa_status = ANY (ARRAY['not_configured', 'pending', 'completed', 'error'])),
...
CREATE TABLE public.external_entities (
  ...
  organization_id uuid NOT NULL,
  display_name text,
  entity_type text NOT NULL DEFAULT 'contact' CHECK (entity_type = ANY (ARRAY['contact', 'company', 'location', 'other'])),
  notes text,
  tags ARRAY,
...
CREATE TABLE public.external_entity_identifiers (
  ...
  organization_id uuid NOT NULL,
  entity_id uuid,
  identifier_type text NOT NULL CHECK (identifier_type = ANY (ARRAY['phone', 'email_domain', 'email', 'crm_object', 'other'])),
...
CREATE TABLE public.external_entity_links (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  link_type text NOT NULL CHECK (link_type = ANY (ARRAY['identifier_to_entity', 'entity_merge', 'entity_split', 'identifier_transfer'])),
...
CREATE TABLE public.external_entity_observations (
  ...
  source_type text NOT NULL CHECK (source_type = ANY (ARRAY['call', 'target', 'campaign_call', 'booking', 'manual'])),
  source_id uuid NOT NULL,
  role text CHECK (role = ANY (ARRAY['caller', 'callee', 'participant', 'target', 'other'])),
  direction text CHECK (direction = ANY (ARRAY['inbound', 'outbound'])),
...
CREATE TABLE public.generated_reports (
  ...
  name text NOT NULL,
  file_path text,
  file_format text CHECK (file_format = ANY (ARRAY['pdf', 'csv', 'xlsx', 'json'])),
  file_size_bytes integer,
  report_data jsonb,
  parameters jsonb NOT NULL DEFAULT '{}',
  generated_by uuid NOT NULL,
  generated_at timestamp with time zone NOT NULL DEFAULT now(),
  status text NOT NULL DEFAULT 'generating' CHECK (status = ANY (ARRAY['generating', 'completed', 'failed'])),
  error_message text,
...
CREATE TABLE public.integrations (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  provider text NOT NULL CHECK (provider = ANY (ARRAY['hubspot', 'salesforce', 'zoho', 'pipedrive'])),
  provider_account_id text,
  provider_account_name text,
  status text NOT NULL DEFAULT 'pending' CHECK (status = ANY (ARRAY['pending', 'active', 'disconnected', 'error', 'expired'])),
  error_message text,
...
CREATE TABLE public.kpi_settings (
  ...
  alert_sensitivity text DEFAULT 'medium' CHECK (alert_sensitivity = ANY (ARRAY['low', 'medium', 'high'])),
  default_test_frequency text DEFAULT '5min' CHECK (default_test_frequency = ANY (ARRAY['5min', '15min', '30min', '1hr', '4hr', '24hr'])),
...
CREATE TABLE public.legal_holds (
  ...
  call_ids ARRAY DEFAULT '{}'[],
  status text NOT NULL DEFAULT 'active' CHECK (status = ANY (ARRAY['active', 'released', 'expired'])),
  effective_from timestamp with time zone NOT NULL DEFAULT now(),
  effective_until timestamp with time zone,
...
CREATE TABLE public.org_sso_configs (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  provider_type text NOT NULL CHECK (provider_type = ANY (ARRAY['saml', 'oidc', 'azure_ad', 'okta', 'google_workspace'])),
  ...
  oidc_scopes ARRAY DEFAULT ARRAY['openid', 'email', 'profile'],
  verified_domains ARRAY DEFAULT '{}'[],
...
CREATE TABLE public.organizations (
  ...
  plan text,
  created_at timestamp with time zone DEFAULT now(),
  plan_status text DEFAULT 'active' CHECK (plan_status = ANY (ARRAY['active', 'past_due', 'canceled', 'trialing', 'incomplete'])),
...
CREATE TABLE public.qa_evaluation_disclosures (
  ...
  call_id uuid NOT NULL,
  organization_id uuid NOT NULL,
  disclosure_type text NOT NULL DEFAULT 'qa_evaluation' CHECK (disclosure_type = ANY (ARRAY['qa_evaluation', 'internal_audit', 'training'])),
  disclosure_text text NOT NULL,
  disclosed_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.recordings (
  ...
  source text NOT NULL DEFAULT 'signalwire' CHECK (source = ANY (ARRAY['signalwire', 'webrtc', 'upload', 'external'])),
...
CREATE TABLE public.report_access_log (
  ...
  user_id uuid NOT NULL,
  action text NOT NULL CHECK (action = ANY (ARRAY['viewed', 'downloaded', 'shared'])),
...
CREATE TABLE public.report_schedules (
  ...
  organization_id uuid,
  test_config_id uuid,
  frequency text NOT NULL CHECK (frequency = ANY (ARRAY['daily', 'weekly'])),
...
CREATE TABLE public.report_templates (
  ...
  name text NOT NULL,
  description text,
  report_type text NOT NULL CHECK (report_type = ANY (ARRAY['call_volume', 'quality_scorecard', 'campaign_performance', 'custom'])),
  data_source text NOT NULL CHECK (data_source = ANY (ARRAY['calls', 'campaigns', 'scorecards', 'surveys', 'multi'])),
...
CREATE TABLE public.retention_policies (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL UNIQUE,
  default_retention_class text NOT NULL DEFAULT 'default' CHECK (default_retention_class = ANY (ARRAY['default', 'regulated', 'legal_hold'])),
...
CREATE TABLE public.scheduled_reports (
  ...
  schedule_time time without time zone NOT NULL DEFAULT '09:00:00' without time zone,
  schedule_days ARRAY,
  timezone text NOT NULL DEFAULT 'UTC',
  delivery_method text NOT NULL DEFAULT 'email' CHECK (delivery_method = ANY (ARRAY['email', 'webhook', 'storage'])),
...
CREATE TABLE public.search_documents (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  source_type text NOT NULL CHECK (source_type = ANY (ARRAY['call', 'recording', 'transcript', 'evidence', 'note'])),
  source_id uuid NOT NULL,
  version integer NOT NULL DEFAULT 1,
...
CREATE TABLE public.search_events (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  event_type text NOT NULL CHECK (event_type = ANY (ARRAY['indexed', 'reindexed', 'rebuild_started', 'rebuild_completed'])),
  document_id uuid,
  source_type text,
  source_id uuid,
  metadata jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  actor_type text NOT NULL CHECK (actor_type = ANY (ARRAY['system', 'human', 'automation'])),
  actor_id uuid,
  actor_label text,
...
CREATE TABLE public.sso_login_events (
  ...
  user_id uuid,
  event_type text NOT NULL CHECK (event_type = ANY (ARRAY['login_success', 'login_failure', 'logout', 'token_refresh'])),
  idp_subject text,
  idp_session_id text,
  email text,
  name text,
...
CREATE TABLE public.stripe_invoices (
  ...
  stripe_subscription_id text,
  status text NOT NULL CHECK (status = ANY (ARRAY['draft', 'open', 'paid', 'void', 'uncollectible'])),
...
CREATE TABLE public.stripe_payment_methods (
  ...
  type text NOT NULL CHECK (type = ANY (ARRAY['card', 'bank_account', 'sepa_debit', 'us_bank_account'])),
...
CREATE TABLE public.stripe_subscriptions (
  ...
  plan text NOT NULL CHECK (plan = ANY (ARRAY['free', 'pro', 'business', 'enterprise'])),
  status text NOT NULL CHECK (status = ANY (ARRAY['active', 'canceled', 'past_due', 'unpaid', 'incomplete', 'incomplete_expired', 'trialing', 'paused'])),
  ...
  currency text NOT NULL DEFAULT 'usd',
  interval text NOT NULL CHECK ("interval" = ANY (ARRAY['month', 'year'])),
...
CREATE TABLE public.systems (
  ...
  description text,
  category text,
  execution_plane text CHECK (execution_plane = ANY (ARRAY['control', 'media', 'ai'])),
...
CREATE TABLE public.team_invites (
  ...
  status text NOT NULL DEFAULT 'pending' CHECK (status = ANY (ARRAY['pending', 'accepted', 'cancelled', 'expired'])),
  invited_by uuid,
  accepted_by uuid,
  created_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.test_statistics (
  ...
  current_status text DEFAULT 'unknown' CHECK (current_status = ANY (ARRAY['healthy', 'warning', 'critical', 'unknown'])),
  ...
  system_id uuid,
  created_by uuid,
  visibility text DEFAULT 'org' CHECK (visibility = ANY (ARRAY['private', 'team', 'org'])),
...
CREATE TABLE public.voice_configs (
  ...
  ai_agent_model text DEFAULT 'gpt-4o-mini' CHECK (ai_agent_model = ANY (ARRAY['gpt-4o-mini', 'gpt-4o', 'gpt-4-turbo'])),
...
CREATE TABLE public.webhook_configs (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid,
  type text NOT NULL CHECK (type = ANY (ARRAY['slack', 'teams'])),
  url text NOT NULL,
  is_active boolean DEFAULT true,
...
CREATE TABLE public.webhook_failures (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid,
  source text NOT NULL CHECK (source = ANY (ARRAY['signalwire', 'assemblyai', 'resend', 'stripe', 'internal'])),
  ...
  status text NOT NULL DEFAULT 'pending' CHECK (status = ANY (ARRAY['pending', 'retrying', 'succeeded', 'failed', 'manual_review', 'discarded'])),
  resolved_at timestamp with time zone,
  resolved_by uuid,
  resolution_notes text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
